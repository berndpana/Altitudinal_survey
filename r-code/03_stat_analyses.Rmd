...
WLqpcr<-hoQpcr[-which(hoQpcr[,"problem_weight_length"]>0),] # discard incomplete weights and length
WLqpcr<-data.frame(subset(WLqpcr,select=c("ho_id","length","weight","type_a","type_b","rel_quant","rel_quant_sd","phytoplasma_type","phytoplasma")))

# discard individuals with incomplete values for weight
Wqpcr<-hoQpcr[-which(hoQpcr[,"problem_weight_length"]==1 | hoQpcr[,"problem_weight_length"]==3),] # discard incomplete weights
Wqpcr<-data.frame(subset(Wqpcr,select=c("ho_id","length","weight","type_a","type_b","rel_quant","rel_quant_sd","phytoplasma_type","phytoplasma")))

# discard individuals with incomplete values for length
Lqpcr<-hoQpcr[-which(hoQpcr[,"problem_weight_length"]==2 | hoQpcr[,"problem_weight_length"]==3),] # discard incomplete lengths
Lqpcr<-data.frame(subset(Lqpcr,select=c("ho_id","length","weight","type_a","type_b","rel_quant","rel_quant_sd","phytoplasma_type","phytoplasma")))
```
...
```{r reg_infect_altitude_body, include=TRUE,echo=TRUE}
